"""
Symbolic pattern grids (4 target layouts) → high-level place/remove tuples.

Use `grid_solver.build_motion_sequence` to turn the resulting plan into robot actions.
Run standalone: ``python -m lab3.pattern_grid_solver``
"""
...
class GridSolver:
    ...
    def plan(self) -> list:
        ...
        for grid in [self.grid1, self.grid2, self.grid3, self.grid4]:
            ...
            if soln is not None:
                plan_leg_f = [
                    ("place", self.colors[grid[i, j]], (i, j), d) for (i, j, d) in soln
                ]
                plan_leg_b = [
                    ("remove", color, coord, d)
                    for (_, color, coord, d) in reversed(plan_leg_f)
                ]
